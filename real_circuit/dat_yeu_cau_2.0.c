...
#define BUZZER_ON() PORTB &= ~(1<<PB7)
#define BUZZER_OFF() PORTB |=(1<<PB7)
#define LED_ON() PORTA |=(1<<PA3)
#define  LED_OFF() PORTA &= ~(1<<PA3)

// Servo position timing ticks based on Timer 0 ISR (1 tick = ~200us)
#define SERVO_POS_MINUS_90 5   // 1.0 ms pulse -> -90 degrees
#define SERVO_POS_0        9   // 1.5 ms pulse -> 0 degrees
#define SERVO_POS_PLUS_90  12  // 2.0 ms pulse -> +90 degrees
...
void init_ADC(void){
	// Select VREF = AVCC (5V), right-adjusted (10-bit)
	ADMUX = (1 << REFS0);
	// Enable ADC, choose Prescaler = 128
	ADCSRA = (1 << ADEN) | (1 << ADPS2) | (1 << ADPS1) | (1 << ADPS0);
}

uint16_t read_ADC(uint8_t channel) {
	// Clear old channel selection bits and set new channel (ADC1)
	ADMUX = (ADMUX & 0xF8) | (channel & 0x07);
	// Start conversion process
	ADCSRA |= (1 << ADSC);
	// Wait until ADSC flag is cleared (conversion complete)
	while (ADCSRA & (1 << ADSC));
...
ISR(TIMER0_OVF_vect) {
	TCNT0 = 233; // chu ki ngat chinh xac là 200us
	static uint8_t servo_tick_counter = 0;
	static uint8_t sensor_tick_divider = 0;  // <--- Them bien bo chia
	static uint16_t alarm_tick_counter = 0;
	
	// --- Software PWM Generation for PE3 Servo (50Hz / 20ms Frame) ---
	servo_tick_counter++;
	if (servo_tick_counter >= 100) {
		servo_tick_counter = 0;
	}
	
	if (servo_tick_counter < servo_pulse_target) {
		PORTE_PORT |= (1 << SERVO_PIN);
		} else {
		PORTE_PORT &= ~(1 << SERVO_PIN);
	}
	
	// --- Background Alarm (LED & Buzzer Toggling) ---
	if (sys_rejected_state == 1) {
		alarm_tick_counter++;
		// 2500 ticks * 200us = 500ms (2/sec)
		if (alarm_tick_counter >= 2500) {
			alarm_tick_counter = 0;
			PORTA ^= (1 << PA3);
			PORTB ^= (1 << PB7);
		}
		} else {
	...
	// Cu 5 ticks ngat (5 * 200us = 1ms) moi xu li cam bien mot lan
	if (sys_rejected_state == 0 && max_num > 0) {
		sensor_tick_divider++;
		if (sensor_tick_divider >= 5) {
			sensor_tick_divider = 0; // Reset bo chia
			process_ir_sensors();    // quet cam bien
		}
	}
}
...
void process_ir_sensors(void) {
	...
	uint16_t adc_val = read_ADC(1);
	
	// STATE 0: Cho vat the moi
	if (scan_state == 0) {
		if (adc_val < 500) {
			scan_state = 1;
			scan_timer = 0;
	...
	if (scan_state == 1) {
		if (adc_val < 80) {
			caught_L = 1;
		} else if (adc_val >= 80 && adc_val < 260) {
			caught_M = 1;
		} else if (adc_val >= 260 && adc_val < 500) {
			caught_S = 1;
		}
		...
		// Chi khi dien ap (> 500) m?i reset v? Tr?ng thái 0
		if (adc_val >= 500) {
			scan_state = 0;
		}
	}
